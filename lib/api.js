...
export async function fetchRouteDetails(routeId) {
  return requestJson(`/api/routes/${routeId}`);
}

export async function fetchStopSchedule(stopId) {
  return requestJson(`/api/stops/${stopId}/schedule`);
}

export async function fetchRouteVehicles(routeId) {
  return requestJson(`/api/routes/${routeId}/vehicles`);
}
...
export function formatDelay(delaySeconds) {
  ...
  return delaySeconds > 0 ? `+${minutes} min late` : `${minutes} min early`;
}
...
export function adaptStationToBusCard(station) {
  ...
  return {
    ...
    minutes: nextArrival?.minutes_until_arrival === 0
      ? 'Arriving'
      : nextArrival?.minutes_until_arrival 
      ?`${nextArrival.minutes_until_arrival} min`
      : estimateWalkingMinutes(station.distance_meters),
    delay: formatDelay(nextArrival?.delay_seconds ?? 0),
    color: getRouteColor(nextArrival?.route_short_name),
    nextArrivals: station.next_arrivals || [],
    routeId: nextArrival?.route_id || null,
  ...
  if (mode === 'unconfigured') {
    return [
      ...
      },
      {
        id: 'demo-delay-late',
        route: 'HOP',
        destination: 'Downtown Boulder',
        stop: 'Broadway & 13th St',
        minutes: '3 min',
        delay: '+8 min late',
        color: getRouteColor('HOP'),
      },
      {
        id: 'demo-delay-early',
        route: 'SKIP',
        destination: 'Front Range/Broadway',
        stop: 'Broadway & Pleasant St',
        minutes: '12 min',
        delay: '2 min early',
        color: getRouteColor('SKIP'),
      },
    ];
  }
