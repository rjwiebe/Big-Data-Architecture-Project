...
JOB_NAME="${JOB_NAME:-rtd-live-data}"
...
POSTGRES_DSN_SECRET="${POSTGRES_DSN_SECRET:-live-data-postgres-dsn}"
...
Next steps:
1. Create a pull request trigger using cloudbuild.live_data.pr.yaml.
2. Create a main branch trigger using cloudbuild.live_data.deploy.yaml.
...
4. Set _REDIS_URL_SECRET, _SCHEDULE, and _TIME_ZONE on the deploy trigger if needed.
